...
impl MaintenanceScheduler {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        for (node_name, node_config) in &self._config.nodes {
            ...
            if let Some(schedule) = &node_config.state_sync_schedule {
                ...
                info!(
                    "State sync enabled but no schedule configured for {}",
                    node_name
                );
            }
        }
        ...
        if scheduled_count > 0 {
            ...
            info!(
                "✓ Maintenance scheduler started successfully with {} jobs",
                scheduled_count
            );
        } else {
                ...
                match http_manager.execute_node_pruning(&node_name).await {
                    Ok(_) => {
                        ...
                        if let Err(e) = database
                            .store_maintenance_operation(&completed_operation)
                            .await
                        {
                    ...
                    Err(e) => {
                        ...
                        if let Err(e) = database
                            .store_maintenance_operation(&failed_operation)
                            .await
                        {
                            error!("Failed to update operation status: {}", e);
                        }
    ...
    async fn schedule_hermes_restart_job(
        ...
        .map_err(|e| {
            anyhow!(
                "Failed to create Hermes restart job for '{}': {}",
                schedule,
                e
            )
        })?;
    ...
    fn validate_6_field_cron(&self, schedule: &str) -> Result<()> {
        ...
        info!(
            "Validated 6-field cron: '{}' → sec:{} min:{} hour:{} day:{} month:{} dow:{}",
