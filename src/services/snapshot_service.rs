// File: src/services/snapshot_service.rs
...
impl SnapshotService {
    ...
    // OPTIMIZED: Inline validations for performance
    #[inline]
    ...
    fn validate_snapshots_enabled(&self, node_name: &str) -> Result<()> {
        let node_config = self.config.nodes.get(node_name).unwrap(); // Already validated above
    ...
    // NEW: Get snapshot-enabled nodes for batch operations
    ...
    // NEW: Get auto-restore-enabled nodes
    ...
    // NEW: Get scheduled snapshot nodes
    ...
    // NEW: Get snapshot service statistics for monitoring
