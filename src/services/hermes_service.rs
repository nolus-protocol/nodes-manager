// File: src/services/hermes_service.rs
...
impl HermesService {
    ...
    pub async fn restart_instance(&self, hermes_name: &str) -> Result<String> {
        ...
        Ok(format!("Hermes instance {} restarted successfully", hermes_name))
    }
    ...
    pub async fn restart_all_instances(&self) -> Result<serde_json::Value> {
        ...
        info!("Restarting all {} Hermes instances", hermes_configs.len());

        let result = self.ssh_manager.restart_multiple_hermes(hermes_configs).await;
    ...
    async fn get_instance_info(&self, hermes_name: &str, hermes_config: &HermesConfig) -> crate::web::HermesInstance {
        ...
        // Get service status
        let status = match self.ssh_manager.check_service_status(&hermes_config.server_host, &hermes_config.service_name).await {
            Ok(service_status) => format!("{:?}", service_status),
            Err(_) => "Unknown".to_string(),
        };

        // Get uptime
        let uptime_formatted = match self.ssh_manager.get_service_uptime(&hermes_config.server_host, &hermes_config.service_name).await {
        ...
        let dependent_nodes = hermes_config.dependent_nodes.clone();
    ...
    pub async fn get_service_statistics(&self) -> Result<serde_json::Value> {
        ...
        for (_hermes_name, hermes_config) in &self.config.hermes {
            ...
            if let Ok(status) = self.ssh_manager.check_service_status(&hermes_config.server_host, &hermes_config.service_name).await {
            ...
            // Check dependencies
            if self.ssh_manager.check_node_dependencies(&hermes_config.dependent_nodes).await.unwrap_or(false) {
