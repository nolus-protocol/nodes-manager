...
pub async fn execute_full_restore_sequence(request: &RestoreRequest) -> Result<String> {
    ...
    commands::execute_shell_command(&data_check_command).await
        .map_err(|_| anyhow::anyhow!("CRITICAL: data directory missing from snapshot: {}/data", request.snapshot_dir))?;

    let wasm_check_command = format!("test -d '{}/wasm'", request.snapshot_dir);
    commands::execute_shell_command(&wasm_check_command).await
        .map_err(|_| anyhow::anyhow!("CRITICAL: wasm directory missing from snapshot: {}/wasm", request.snapshot_dir))?;
    ...
    let data_size = commands::execute_shell_command(&data_size_command).await
        .unwrap_or_default()
        .trim()
        .parse::<u64>()
        .unwrap_or(0);
    ...
    let wasm_size = commands::execute_shell_command(&wasm_size_command).await
        .unwrap_or_default()
        .trim()
        .parse::<u64>()
        .unwrap_or(0);

    info!("Snapshot data size: {:.1} MB, wasm size: {:.1} MB",
    ...
    commands::execute_shell_command(&verify_data_cmd).await
        .map_err(|_| anyhow::anyhow!("CRITICAL: data directory not found after copy to {}/data", request.deploy_path))?;

    let verify_wasm_cmd = format!("test -d '{}/wasm'", request.deploy_path);
    commands::execute_shell_command(&verify_wasm_cmd).await
        .map_err(|_| anyhow::anyhow!("CRITICAL: wasm directory not found after copy to {}/wasm", request.deploy_path))?;
    ...
    let wasm_chown_cmd = format!("chown -R $(stat -c '%U:%G' '{}') '{}/wasm'",
                                request.deploy_path, request.deploy_path);
